---
description: "面向应用表面与可选策略层的现成 dsh profile 组合包，包括有界的本地模型压缩。"
kind: "package-group"
---
...
本组列出 `dsh --profile` 使用的可安装 patch 层。每个包都声明 `dsh.bundle.patch`；启动器会叠放这些 patch 文档来组装具名 profile。`web`、`headless`、`acp` 与 `sdk` profile 以 `dsh-base` 为基础，`sdk-minimal` 则由一个组合包提供完整配置树。`resilient-compaction` 是面向本地或吞吐受限模型的可选策略层。领域包也可以在本目录之外声明附加层。
...
| [`resilient-compaction`](resilient-compaction/README.zh.md) | 基于 base 的有界压缩与分派前请求准入 | 挂载全局 LLM 策略 |
