...
- **Fork customization:** `master` mirrors `deepseek-official/master`; persistent work targets `custom/main`. Register every delta in [the customization policy](.agents/customization-policy.json), prefer opt-in extensions, and merge `master` into the custom branch ([procedure](docs/customization/upstream-safe-customization.md)).
