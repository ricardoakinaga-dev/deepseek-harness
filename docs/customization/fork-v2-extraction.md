...
| Hierarchical multi-call summarization | Current `compaction/summary.llmStreamCall` identifies one auxiliary call | Defer until upstream session events can represent every intermediate call; never hide calls behind the one-call marker |
